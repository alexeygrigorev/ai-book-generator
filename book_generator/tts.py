...
class TTSGenerator:
    def __init__(self, model: str = "models/gemini-2.5-flash-preview-tts", voice_name: str = "Charon", num_threads: int = 10, s3_bucket: Optional[str] = None):
        self.model = model
    ...
    def _create_wave_bytes(self, pcm_data: bytes, channels: int = 1, rate: int = 24000, sample_width: int = 2) -> bytes:
    ...
    def _process_single_file(self, file_path: Path, base_path: Path, audio_base_path: Path, book_name: str):
    ...
    def _process_book_standard(self, base_path: Path, audio_base_path: Path, book_name: str, limit: Optional[int] = None):
        ...
        if self.num_threads == 1:
            for file_path in tqdm(files_to_process, desc="Generating Audio"):
                self._process_single_file(file_path, base_path, audio_base_path, book_name)
            return
        ...
        try:
            with tqdm(total=len(files_to_process), desc="Generating Audio") as progress:
                ...
                for file_path in files_to_process:
                    future = pool.submit(self._process_single_file, file_path, base_path, audio_base_path, book_name)
