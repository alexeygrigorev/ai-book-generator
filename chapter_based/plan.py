...
from chapter_based.models import BookPlan
from book_generator.utils import get_client, calculate_gemini_3_cost

planner_instructions = """
Your role is planning the book.

You're given a conversation between a user and an assistant about a book. Based
on the conversation, you need to create a detailed book plan with each chapter.
Later we will give this to the writer, who will write one full chapter at a time.

A part should have at least 3-4 chapters, and each chapter should have at least 7-8 bullet
points outlining what the chapter will cover.
...
Do not add numbers to chapter names. They will be added later automatically.
...
Please provide a comprehensive book outline including parts (if applicable) and chapters with brief descriptions.
Each chapter should have 7-8 bullet points outlining the key topics to cover."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate a structured chapter-based book plan from a prompt file."
    )
    parser.add_argument(
        "-p",
        "--prompt-file",
